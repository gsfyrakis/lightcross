...
def create_csv(output_filename, formatted_output, other_content=None):
    with open('output.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        field = ['Tool', 'File', 'Contract', 'Vulnerability', 'Severity', 'SWC-ID', 'Remediation',
                 'Description/More Info', 'Execution time']
...
def analyse_smart_contract_with_slither(file_path):
    try:
        ...
        with open('syslog-slither.txt', 'w') as f:
            print(file_path)
            start_time_slither = time.time()
            proc = subprocess.run(['slither', file_path], stdout=f, stderr=f, text=True)
...
def analyse_smart_contract_with_mythril(file_path):
    ...
    try:
        ...
        with open('syslog-mythril.txt', 'w') as f:
            start_time_mythril = time.time()
            proc = subprocess.run(['myth', 'analyze', file_path], stdout=f, stderr=f, text=True)
...
def main():
    ...
    banner_content = (
        "=========================================================\n"
        "   Solidity Smart Contract Cross-Tool Vulnerability Detector \n"
        "=========================================================\n\n"
    )
    contract_files = []
